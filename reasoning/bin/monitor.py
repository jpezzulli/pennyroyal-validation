...
def main():
    ...
    parser.add_argument("--heat-sentinel")
    ...
    with open(args.output, "w", newline="", buffering=1) as handle:
        ...
        while not STOP:
            ...
            cpu_peak = max(
                value for value in (
                    sensors.get("cpu0_temp_c"), sensors.get("cpu1_temp_c")
            ...
            gpu_temp = gpu0.get("temp_c")
            if args.heat_sentinel and (
                (cpu_peak is not None and cpu_peak >= args.cpu_limit)
                or (gpu_temp is not None and gpu_temp >= args.gpu_limit)
            ):
                payload = {
                    "epoch": started,
                    "cpu_peak_c": cpu_peak,
                    "gpu_temp_c": gpu_temp,
                    "cpu_limit_c": args.cpu_limit,
                    "gpu_limit_c": args.gpu_limit,
                }
                Path(args.heat_sentinel).write_text(json.dumps(payload) + "\n")
            prev_time = started
