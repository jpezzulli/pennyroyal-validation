...
SUITE_ID = "reasoning-context-v2"
RUBRIC_PATH = "cases/reasoning-context-v2-rubric.json"


class EqualityTokenizer:
    """Dependency-free token equality for loop detection.

    Loop detection needs stable repeated units, not checkpoint token IDs. Server
    usage remains authoritative for all reported token counts.
    """
...
def measured_payload(model, request_messages, reasoning_effort=None):
...
def measured_plan(suite, selected_case_ids=None):
    plan = []
...
def execution_waves(suite, profile, selected_case_ids=None):
    ...
    if profile == SEQUENTIAL_PROFILE:
        ...
        if selected_case_ids:
            raise ValueError("targeted --case runs require the sequential profile")
...
def collect_case(
    ...
    *,
    ...
    payload = measured_payload(
        model,
        messages(suite.SYSTEM, case["prompt"]),
        reasoning_effort,
    )
    ...
    correction_payload = measured_payload(
        model, correction_messages, reasoning_effort
    )
...
def replay_manifest(path, suite):
    ...
    manifest = {
        ...
        "grading": (
            f"Responses require blinded qualitative grading with {RUBRIC_PATH}; "
...
def main():
    ...
    parser.add_argument(
        ...
        help="optional request-level reasoning effort override",
    )
    parser.add_argument(
        "--case",
        dest="case_ids",
        action="append",
        choices=[f"C{index}" for index in range(1, 9)],
        help="run only the selected measured case; repeat for multiple cases",
    )
    parser.add_argument(
        "--execution-profile",
    ...
    selected_case_ids = list(dict.fromkeys(args.case_ids or []))
    selected_cases = [
        case
        for case in suite.CASES
        if not selected_case_ids or case["id"] in selected_case_ids
    ]
    plan = measured_plan(suite, selected_case_ids)
    waves = execution_waves(suite, args.execution_profile, selected_case_ids)
    if args.list:
    ...
    for wave in waves:
        ...
        if any(result["error"] for result in wave_results):
            return 2
        if any(result.get("finish_reason") == "length" for result in wave_results):
            append_jsonl(events_path, {
                "event": "invalid_output_budget_truncation",
                "request_ids": [
    ...
    measured_rows = [
        row for row in read_jsonl(results_path) if row.get("kind") == "measured"
    ]
    truncated = [
        row.get("request_id")
    ...
    write_json(out / "manifest.json", {
        ...
        "execution_profile": args.execution_profile,
        "execution_waves": waves,
        "planned_measured_requests": len(plan),
        "measured_requests": len(measured_rows),
        "measured_max_tokens": None,
        "measured_max_tokens_field": "omitted",
        "reasoning_effort": args.reasoning_effort,
        "loop_tokenizer": (
            f"transformers:{args.tokenizer_path}"
            if args.tokenizer_path
            else "stdlib-word-punctuation-equality"
        ),
        "truncated_requests": truncated,
        "qualification_valid": not truncated,
        "request_plan": plan,
        "results": "results.jsonl",
        "grading": f"blinded qualitative review required; see {RUBRIC_PATH}",
    })
    if truncated:
        print(
            "INVALID: measured requests reached a finite output-token limit: "
            + ", ".join(truncated),
            file=sys.stderr,
        )
        return 3
